...
void update_threads_recent_cpu(){
    ...
    for (e = list_begin(&all_list); e != list_end(&all_list); e = list_next(e)) {
        ...
        // term2 = 2*load_avg + 1
        fixed_t term2 = real_multiply_by_int(&load_avg, 2);
        term2 = real_add_to_int(&term2, 1);

        // Combine term1 and term2 for the final calculation of recent_cpu
        term1 = real_multiply(&term1, &term2);
...
void update_load_avg() {
    ...
    coefficient = real_multiply_by_int(&coefficient, list_size(&ready_list));
